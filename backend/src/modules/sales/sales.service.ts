...
export async function list(user: AuthenticatedUser, query: ListSalesQuery) {
  ...
  return paginatedResult(data, total, { page: query.page, perPage: query.perPage })
}
...
export async function create(user: AuthenticatedUser, input: CreateSaleInput) {
  ...
  if (input.produtos?.length) {
    for (const item of input.produtos) {
      await addItem(user, venda.id, { produtoId: item.produtoId, quantidade: item.quantidade })
    }
  } else {
...
export async function addItem(user: AuthenticatedUser, vendaId: string, input: AddItemInput) {
  const venda = await ensureSaleEditable(user.empresaId, vendaId)

  const produto = input.produtoId
    ? await prisma.produto.findFirst({
        where: { id: input.produtoId, empresaId: user.empresaId, deletedAt: null },
      })
    : await prisma.produto.findFirst({
        where: { codigo: input.codigo!, empresaId: user.empresaId, deletedAt: null },
      })
  ...
  if (existing) {
    ...
    await prisma.vendaProduto.update({
      ...
      data: {
        ...
        subtotal: lineSubtotal({ precoUnitario: produto.preco, quantidade: novaQtd }),
      },
    })
  } else {
    await prisma.vendaProduto.create({
      data: {
        ...
        subtotal: lineSubtotal({ precoUnitario: produto.preco, quantidade: input.quantidade }),
      },
    })
  }
...
export async function removeItem(user: AuthenticatedUser, vendaId: string, produtoId: string) {
  const venda = await ensureSaleEditable(user.empresaId, vendaId)
...
export async function setDiscount(user: AuthenticatedUser, vendaId: string, desconto: number) {
  const venda = await ensureSaleEditable(user.empresaId, vendaId)
...
export async function finalize(
  ...
  if (!items) throw new BadRequestError('Não é possível finalizar venda sem itens')
...
export async function cancel(user: AuthenticatedUser, vendaId: string) {
  ...
  if (!venda) throw new NotFoundError('Venda não encontrada')
  if (venda.status === 'CANCELADA') throw new ConflictError('Venda já está cancelada')
...
async function ensureSaleEditable(empresaId: string, vendaId: string): Promise<Venda> {
  const venda = await prisma.venda.findFirst({ where: { id: vendaId, empresaId } })
  if (!venda) throw new NotFoundError('Venda não encontrada')
  if (venda.status !== 'ABERTA') {
    throw new ConflictError(`Venda não pode ser editada (status: ${venda.status})`)
  }
  return venda
}
...
async function recalcAndPersist(vendaId: string) {
  const [venda, items] = await Promise.all([
    prisma.venda.findUnique({ where: { id: vendaId }, select: { desconto: true } }),
