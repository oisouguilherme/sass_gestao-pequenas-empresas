...
function buildAccessFilter(user: AuthenticatedUser): Prisma.OrdemServicoWhereInput {
...
export async function list(user: AuthenticatedUser, query: ListOrdersQuery) {
  const where: Prisma.OrdemServicoWhereInput = {
    ...
    ...(query.usuarioId && { usuarios: { some: { usuarioId: query.usuarioId } } }),
...
export async function create(user: AuthenticatedUser, input: CreateOrderInput) {
  ...
  if (usuarioIds.length) await ensureUsuariosBelongToEmpresa(user.empresaId, usuarioIds)
  if (produtos.length) await ensureProdutosBelongToEmpresa(user.empresaId, produtos.map((p) => p.produtoId))
  ...
  const os = await prisma.ordemServico.create({
    data: {
      ...
      produtos: produtos.length
        ? { create: produtos.map((p) => ({ produtoId: p.produtoId, quantidade: p.quantidade })) }
        : undefined,
    },
    include: ORDER_INCLUDE,
  })
  ...
  if (usuarioIds.length) notifyAssignedUsers(os.id, usuarioIds).catch(() => undefined)

  return os
}

export async function update(user: AuthenticatedUser, id: string, input: UpdateOrderInput) {
...
export async function updateStatus(user: AuthenticatedUser, id: string, status: OSStatus) {
  await assertOSBelongsToEmpresa(user.empresaId, id)
  return prisma.ordemServico.update({
    where: { id },
    data: { status },
...
export async function setUsuarios(user: AuthenticatedUser, id: string, usuarioIds: string[]) {
  await assertOSBelongsToEmpresa(user.empresaId, id)
...
export async function setProdutos(
  ...
  await assertOSBelongsToEmpresa(user.empresaId, id)
  await ensureProdutosBelongToEmpresa(user.empresaId, produtos.map((p) => p.produtoId))
...
async function ensureUsuariosBelongToEmpresa(empresaId: string, usuarioIds: string[]) {
...
async function ensureProdutosBelongToEmpresa(empresaId: string, produtoIds: string[]) {
